...
export default function FinancialInstiContactModal({
  opened,
  onClose,
  contact,
  onSave,
  ...
  const isEditing = !!contact?.contact_details_id;

  const form = useForm({
    initialValues: {
      contact_type: "Mobile",
      contact_detail: "",
      financial_insti_ID: financial_insti_ID || "",
    },
    validate: {
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      if (isEditing) {
        ...
        savedContact = await updateInstitutionContact(
        ...
        // Add new contact
        savedContact = await addfinancialInsti_contacts({
          contact_type: values.contact_type,
          contact_detail: values.contact_detail,
          financial_insti_id: financial_insti_ID,
        });
      ...
      <form onSubmit={form.onSubmit(handleSubmit)}>
        <Stack spacing="md">
          ...
          <TextInput
            ...
            type={
              form.values.contact_type === "Email"
                ? "email"
                : form.values.contact_type === "Mobile" ||
                  form.values.contact_type === "Landline"
                ? "tel"
                : "text"
            }
            placeholder={
              form.values.contact_type === "Email"
                ...
                : form.values.contact_type === "Mobile"
                ? "e.g 09123456789 (11 digits)"
                : form.values.contact_type === "Landline"
                ? "e.g 2345678 (7-9 digits)"
            ...
            onKeyPress={(event) => {
              ...
              // Only numbers allowed for Mobile/Landline
              if (
              ...
              // Enforce max length
              if (
                (type === "Mobile" && value.length >= 11) ||
                (type === "Landline" && value.length >= 9)
              ) {
                event.preventDefault();
              }
            }}
